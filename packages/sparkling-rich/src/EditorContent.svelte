<script lang="ts">
  ...
  export let transformInjections: (state: EditorState) => Promise<Transaction>;
  ...
  view = new EditorView(rootElement, {
    ...
    dispatchTransaction(transaction) {
      ...
      if (transformInjections != null) {
        let tr: Promise<Transaction> = transformInjections(newState);
        if (tr != null) {
          tr.then((res) => {
            if (res != null) {
...
{/if}
<div class="edit-box" bind:this={root} />
